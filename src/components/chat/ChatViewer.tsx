    ...
    <div className={`block ${blockClass}`}>
      ...
      >
        ...
        <span className="block-icon">
          {type === "thinking" ? (
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={accentColor} strokeWidth="2" aria-hidden="true">
...
// Block Renderer
function BlockView({ block, isUserMessage }: { block: ChatBlock; isUserMessage?: boolean }) {
  if (block.type === "thinking") {
    return (
      <CollapsibleBlock
        type="thinking"
        label={block.label || "Thinking..."}
        content={block.text}
  ...
  // Default: text block with bubble style for user messages
  return (
    <div className={`block block-text ${isUserMessage ? "user-text" : ""}`}>
      {block.text}
    </div>
  );
}

// Message Component - Bubble Style
    ...
    <article className={`message ${isUser ? "message-user" : "message-assistant"}`}>
      ...
      <div className="message-content">
        ...
        <div className="message-meta">
          <span className="message-role">{getRoleLabel(turn.role)}</span>
          {turn.timestamp && (
        ...
        <div className="chat-content">
          ...
          <header className="chat-header">
            ...
            <div className="chat-header-actions">
              <button
                type="button"
                className={`chat-header-btn ${showInspector ? "active" : ""}`}
